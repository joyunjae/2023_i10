...
static inline bool i10_host_has_inline_data(struct i10_host_request *req)
{
	...
	return rq_data_dir(rq) == WRITE && bytes &&
		bytes <= i10_host_inline_data_size(req->queue);
}
		...
		struct nvme_completion *cqe)
{
	...
	rq = nvme_find_rq(i10_host_tagset(queue), cqe->command_id);
	if (!rq) {
		dev_err(queue->ctrl->ctrl.device,
			"got bad cqe.command_id %#x on queue %d\n",
			cqe->command_id, i10_host_queue_id(queue));
		...
		struct nvme_tcp_data_pdu *pdu)
{
	...
	rq = nvme_find_rq(i10_host_tagset(queue), pdu->command_id);
	if (!rq) {
		dev_err(queue->ctrl->ctrl.device,
			"got bad c2hdata.command_id %#x on queue %d\n",
			pdu->command_id, i10_host_queue_id(queue));
		return -ENOENT;
	}
		...
		struct nvme_tcp_rsp_pdu *pdu)
{
	 ...
	 */
	if (unlikely(nvme_is_aen_req(i10_host_queue_id(queue),
					cqe->command_id)))
		...
		unsigned int *offset, size_t *len)
{
	...
	*len -= rcv_len;
	...
	case nvme_tcp_c2h_data:
		ret = i10_host_handle_c2h_data(queue, (void *)queue->pdu);
		break;
	case nvme_tcp_rsp:
		i10_host_init_recv_ctx(queue);
		ret = i10_host_handle_comp(queue, (void *)queue->pdu);
		break;
	case nvme_tcp_r2t:
		i10_host_init_recv_ctx(queue);
		ret = i10_host_handle_r2t(queue, (void *)queue->pdu);
		break;
	default:
		dev_err(queue->ctrl->ctrl.device,
			      ...
			      unsigned int *offset, size_t *len)
{
	...
	struct request *rq =
		nvme_cid_to_rq(i10_host_tagset(queue), pdu->command_id);
	struct i10_host_request *req = blk_mq_rq_to_pdu(rq);
		...
		struct sk_buff *skb, unsigned int *offset, size_t *len)
{
	...
	*len -= recv_len;
	...
	if (pdu->hdr.flags & NVME_TCP_F_DATA_SUCCESS) {
		struct request *rq = nvme_cid_to_rq(i10_host_tagset(queue),
					pdu->command_id);
...
static void i10_host_fail_request(struct i10_host_request *req)
{
	struct request *rq = blk_mq_rq_from_pdu(req);
	union nvme_result res = {};
	u16 status = NVME_SC_HOST_PATH_ERROR;
...
static int i10_host_try_send_data(struct i10_host_request *req)
{
	...
	while (true) {
		...
		if (i10_host_legacy_path(req)) {
			if (sendpage_ok(page))
				ret = kernel_sendpage(queue->sock, page, offset,
						len, flags);
			else
				ret = sock_no_sendpage(queue->sock, page, offset,
						len, flags);
		}
		else {
		...
		int qid, size_t queue_size)
{
	...
	ret = sock_setsockopt(queue->sock, SOL_SOCKET, SO_SNDBUFFORCE,
			optval, sizeof(opt));
	if (ret) {
		dev_err(ctrl->ctrl.device,
	...
	ret = sock_setsockopt(queue->sock, SOL_SOCKET, SO_RCVBUFFORCE,
			optval, sizeof(opt));
	if (ret) {
		dev_err(ctrl->ctrl.device,
...
static int nvme_alloc_io_queues(struct nvme_ctrl *ctrl)
{
	...
	return i10_host_alloc_io_queues(ctrl);
}
...
// 네트워크 전송을 위한 커맨드 PDU를 설정하는 역할
static blk_status_t i10_host_setup_cmd_pdu(struct nvme_ns *ns,
		struct request *rq)
{
	struct i10_host_request *req = blk_mq_rq_to_pdu(rq); // request 포인터의 1byte를 건너뛴 pointer를 다시 return해줌
	struct nvme_tcp_cmd_pdu *pdu = req->pdu; // request에서 pdu 정보 가져옴
	struct i10_host_queue *queue = req->queue; // 해당 request에 대한 queue 정보
	u8 hdgst = i10_host_hdgst_len(queue), ddgst = 0;
	blk_status_t ret;

	ret = nvme_setup_cmd(ns, rq, &pdu->cmd);//NVMe(SSD) device에 전달될 command를 설정하는 역할
	if (ret)
		return ret;
		...
		const struct blk_mq_queue_data *bd)
{
	...
	struct i10_host_queue *queue = hctx->driver_data; // 현재의 i10-lane
	struct request *rq = bd->rq; // Block I/O request 뽑아오고
	struct i10_host_request *req = blk_mq_rq_to_pdu(rq); // *request의 1byte 건너뛴 pointer 저장하고
	...
	ret = i10_host_setup_cmd_pdu(ns, rq); // request를 네트워크 전송을 위한 command PDU로 설정
	if (unlikely(ret))
		return ret;

	blk_mq_start_request(rq); // 원래 커널 코드 - Block I/O 시작했음을 알리기 위한 state들 update하는

	i10_host_queue_request(req); // i10 queue에 request 던지기 시작
